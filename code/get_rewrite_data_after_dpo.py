...
def main(args):
    ...
    os.system(f"rm -rf {os.path.dirname(args.output_file)}/ada/test*")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_path', type=str, default='{project_root}/models/going0523/G2_cat_bm25', help='Path to the model')
    ...
    #批量测试参数
    parser.add_argument('--model_dir', type=str, default='{project_root}/models/my_bridge_model/grpo_0629', help='Path to the multi_model')
    parser.add_argument('--data_dir', type=str, default='{project_root}/data/retrieval-test200', help='Path to the multi_model')
    parser.add_argument('--save_dir', type=str, default='{project_root}/data/retrieval-test200-grpo_0629', help='')
    ...
    for dataset_name_w_retriever in os.listdir(args.model_dir):
        if "checkpoint" in os.listdir(os.path.join(args.model_dir, dataset_name_w_retriever))[0]:
            for ckpt_name in os.listdir(os.path.join(args.model_dir, dataset_name_w_retriever)):
                args.model_path =  os.path.join(args.model_dir, dataset_name_w_retriever, ckpt_name)    
                dataset_name, retriever_type = dataset_name_w_retriever.split("_")
                ...
                for sub_dataset_name in os.listdir(args.data_dir):
                    if sub_dataset_name.split("_")[0] == dataset_name:
                        ...
                        args.output_file = os.path.join(args.save_dir, sub_dataset_name, "fuzzy_rewrite_by_"+args.model_dir.split("/")[-1]+"_"+retriever_type+"_"+ckpt_name, "test.query.txt")
                        main(args)
        else:
            # 单checkpoint
            args.model_path = os.path.join(args.model_dir, dataset_name_w_retriever)
            dataset_name, retriever_type = dataset_name_w_retriever.split("_")
            
            for sub_dataset_name in os.listdir(args.data_dir):
                if sub_dataset_name.split("_")[0] == dataset_name:
                    args.input_file = os.path.join(args.data_dir, sub_dataset_name, "fuzzy", "test.query.txt")
                    args.output_file = os.path.join(args.save_dir, sub_dataset_name, "fuzzy_rewrite_by_"+args.model_dir.split("/")[-1]+"_"+retriever_type, "test.query.txt")
                    main(args)
